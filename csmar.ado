...
program define csmar
    syntax anything, [REmove case(string) Allstring otheropt(string)]
    ...
    if (`filematch' == 0){
        ...
        foreach f in `r(files)'{
            ...
            csmar_xlsx "`f'", case("`case'") `allstring' clear otheropt("`otheropt'")
        ...
        csmar_xlsx `anything', case("`case'") `allstring' clear otheropt("`otheropt'")
        qui timer off 1
        dis as res "==================  {it:End}  =================="
        qui timer list 1
        local tt: dis %10.2f `r(t1)'
...
program define csmar_xlsx
    syntax anything, [case(string) Allstring clear otheropt(string)]
    ...
    qui import excel using `anything', firstrow `allstring' case(`case') `clear' `otheropt'
